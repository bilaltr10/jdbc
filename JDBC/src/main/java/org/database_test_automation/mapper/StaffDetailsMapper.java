...
public class StaffDetailsMapper implements RowMapper<StaffDetails> {
	...
	public StaffDetails mapRow(ResultSet rs) throws Exception {
		return new StaffDetails(rs.getInt("staff_id"), rs.getString("first_name"), rs.getString("last_name"),
				rs.getInt("address_id"), rs.getString("email"), rs.getInt("store_id"), rs.getBoolean("active"),
				rs.getString("username"), rs.getString("password"), rs.getTimestamp("last_update"),
