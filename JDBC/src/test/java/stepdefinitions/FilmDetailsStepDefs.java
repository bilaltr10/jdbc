...
public class FilmDetailsStepDefs {
    ...
    public void ı_should_receive_a_list_containing_all_films_in_the_database() {
        assertThat(filmDetailsList)
                .as("Check if the film list is not null and not empty")
                .isNotNull()
                .isNotEmpty();
    }
    ...
    public void all_the_films_appeared_should_contain(String filmName) {
        ...
        if (filmDetailsList != null) {
            for (FilmDetails filmDetails : filmDetailsList) {
                softAssertions.assertThat(filmDetails.getTitle()).as("film should contains eword of" + filmName).contains(filmName);
            }
    ...
    public void ıShouldReceiveTheTitleAndDescriptionOfTheFilmWithID(String filmId, String filmTitle, String filmDescription) {
